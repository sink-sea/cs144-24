...
class Host {
  ...
  public:
    ...
    Host(string name, const Address& my_address, const Address& next_hop, shared_ptr<NetworkSegment> network)
      ...
      , _interface(make_shared<NetworkInterface>(_name, move(network), random_host_ethernet_address(), _my_address))
      , _next_hop(next_hop) {}
    ...
    void check() {
        while (not _interface->datagrams_received().empty()) {
            ...
            if (not expecting(dgram_received)) {
                throw runtime_error(
                  "Host " + _name + " received unexpected Internet datagram: " + dgram_received.header.to_string());
            }
  ...
  public:
    Network()
      ...
      , eth1_id(_router.add_interface(
          make_shared<NetworkInterface>("eth1", empty, random_router_ethernet_address(), Address {"172.16.0.1"})))
      , eth2_id(_router.add_interface(make_shared<NetworkInterface>("eth2",
                                                                    eth2_cherrypie,
                                                                    random_router_ethernet_address(),
                                                                    Address {"192.168.0.1"})))
      , uun3_id(_router.add_interface(
          make_shared<NetworkInterface>("uun3", uun, random_router_ethernet_address(), Address {"198.178.229.1"})))
      ...
      , mit5_id(_router.add_interface(make_shared<NetworkInterface>("mit5",
        ...
        _hosts.insert({"applesauce", {"applesauce", Address {"10.0.0.2"}, Address {"10.0.0.1"}, eth0_applesauce}});
        _hosts.insert({"default_router", {"default_router", Address {"171.67.76.1"}, Address {"0"}, upstream}});
        _hosts.insert(
